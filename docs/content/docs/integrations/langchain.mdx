...
The `@slug/model` format routes requests through a specific provider configured in your `providers` array. The `@openai/` prefix routes through the provider with slug `openai`. You can use any configured slug (e.g., `@openai/gpt-4o-mini`, `@groq/llama-3.3-70b`).

> **Note:** Divergent providers (e.g. `anthropic`) require a dedicated gateway adapter via `config.adapters`. Only OpenAI-compatible providers can be routed directly.
