---
title: Configuration
description: Learn about all configuration options for LLMOps.
icon: Settings
---

The `llmops()` function accepts a configuration object that controls how LLMOps behaves. This page documents all available options.

## Basic Example

```typescript title="llmops.ts"
import { llmops, basicAuth } from '@llmops/sdk';
import { Pool } from 'pg';

export default llmops({
  basePath: '/llmops',
  providers: {
    openai: { apiKey: process.env.OPENAI_API_KEY },
  },
  database: new Pool({
    connectionString: process.env.DATABASE_URL,
  }),
  auth: basicAuth({
    username: 'admin@example.com',
    password: 'password',
  }),
});
```

---

## Configuration Options

### basePath

The URL path where LLMOps routes will be mounted. Must start with `/`.

```typescript
basePath: '/llmops';
```

This determines where the dashboard and API endpoints are accessible:

- Dashboard: `http://localhost:3000/llmops`
- API: `http://localhost:3000/llmops/api/genai/v1/chat/completions`

---

### providers

An object containing your LLM provider configurations. At least one provider must be configured.

```typescript
providers: {
  openai: {
    apiKey: process.env.OPENAI_API_KEY,
  },
  anthropic: {
    apiKey: process.env.ANTHROPIC_API_KEY,
  },
}
```

See [Providers](/docs/providers) below for detailed provider-specific options.

---

### database

A PostgreSQL connection pool instance.

```typescript
import { Pool } from 'pg';

database: new Pool({
  connectionString: process.env.DATABASE_URL,
});
```

You can also pass individual connection options:

```typescript
database: new Pool({
  host: 'localhost',
  port: 5432,
  database: 'llmops',
  user: 'postgres',
  password: 'password',
});
```

---

### auth

Authentication configuration for the LLMOps dashboard and API. Use the `basicAuth()` helper function to configure basic authentication.

```typescript
import { basicAuth } from '@llmops/sdk';

auth: basicAuth({
  username: 'admin@example.com',
  password: 'secure-password',
});
```

| Property   | Type     | Description                 |
| ---------- | -------- | --------------------------- |
| `username` | `string` | Username for authentication |
| `password` | `string` | Password for authentication |

The `basicAuth()` function validates that both username and password are provided and returns a properly typed configuration object.

---

### autoMigrate

Controls automatic database migration behavior on startup.

| Value           | Behavior                                               |
| --------------- | ------------------------------------------------------ |
| `false`         | Never run migrations automatically (default)           |
| `true`          | Always run migrations on startup                       |
| `'development'` | Only run migrations when `NODE_ENV` is `'development'` |

```typescript
// Only auto-migrate in development
autoMigrate: 'development';
```

For production deployments, we recommend running migrations manually using the CLI instead of `autoMigrate: true`.

---

### schema

The PostgreSQL schema name where LLMOps tables will be created. Defaults to `'llmops'`.

```typescript
schema: 'llmops'; // default
```

This option:

- Creates the schema if it doesn't exist (during migrations)
- Sets `search_path` on every database connection
- Isolates LLMOps tables from your application's tables

| Value      | Behavior                                  |
| ---------- | ----------------------------------------- |
| `'llmops'` | Uses dedicated `llmops` schema (default)  |
| `'public'` | Uses the default PostgreSQL public schema |
| `'custom'` | Uses a custom schema name of your choice  |

```typescript
// Use public schema (not recommended for production)
schema: 'public';

// Use a custom schema
schema: 'my_llmops_schema';
```

Using a dedicated schema keeps LLMOps tables separate from your application tables, making it easier to manage and backup.

---

## Providers Configuration

All providers share these base options:

| Property         | Type     | Required | Description                      |
| ---------------- | -------- | -------- | -------------------------------- |
| `apiKey`         | `string` | Yes      | API key for the provider         |
| `customHost`     | `string` | No       | Custom base URL for the provider |
| `requestTimeout` | `number` | No       | Request timeout in milliseconds  |

```typescript
providers: {
  openai: {
    apiKey: process.env.OPENAI_API_KEY,
  },
  anthropic: {
    apiKey: process.env.ANTHROPIC_API_KEY,
  },
  openrouter: {
    apiKey: process.env.OPENROUTER_API_KEY,
  },
}
```

You can configure multiple providers to use different models based on your needs. Once configured, you can create variants in the dashboard that use any of these providers.

For detailed configuration options for each provider, see the [Providers](/docs/providers) page.

---

## Complete Example

```typescript title="llmops.ts"
import 'dotenv/config';
import { llmops, basicAuth } from '@llmops/sdk';
import { Pool } from 'pg';

export default llmops({
  basePath: '/llmops',

  providers: {
    openai: {
      apiKey: process.env.OPENAI_API_KEY!,
    },
    anthropic: {
      apiKey: process.env.ANTHROPIC_API_KEY!,
    },
    openrouter: {
      apiKey: process.env.OPENROUTER_API_KEY!,
    },
  },

  database: new Pool({
    connectionString: process.env.DATABASE_URL!,
  }),

  auth: basicAuth({
    username: process.env.LLMOPS_USER || 'admin@example.com',
    password: process.env.LLMOPS_PASSWORD || 'password',
  }),

  // Auto-migrate only in development
  autoMigrate: 'development',

  // PostgreSQL schema (defaults to 'llmops')
  schema: 'llmops',
});
```

---

## Environment Variables

We recommend storing sensitive values in environment variables:

```bash title=".env"
# Database
DATABASE_URL=postgresql://postgres:password@localhost:5432/llmops

# Providers
OPENAI_API_KEY=sk-...
ANTHROPIC_API_KEY=sk-ant-...
OPENROUTER_API_KEY=sk-or-...

# Auth
LLMOPS_USER=admin@example.com
LLMOPS_PASSWORD=secure-password
```
