...
import { ChevronRight, Columns2, Database, Plus } from 'lucide-react';
import { DatasetsDataTable } from './datasets/-components/datasets-data-table';
...
function RouteComponent() {
  ...
  const handleNavigateToNew = () => {
    navigate({ to: '/datasets/$id', params: { id: 'new' } });
  };
      ...
      <div className={gridElement}>
        <div ref={containerRef} className={twinSplitContainer}>
          ...
          <div className={clsx(workingArea, rightTile)}>
            <div>
              <DatasetsHeader id={params.id as string} />
              <Outlet />
            </div>
          </div>
        </div>
