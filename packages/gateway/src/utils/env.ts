...
// Load Node.js built-ins lazily via dynamic import (no require/createRequire)
const nodeModulesReady = isNodeInstance
  ? Promise.all([import('node:path'), import('node:fs')]).then(([p, f]) => {
      nodePath = p;
      nodeFs = f;
    })
  : Promise.resolve();
  ...
  try {
    if (
      ...
      if (nodeFs.existsSync(resolvedPath)) {
        return nodeFs.readFileSync(resolvedPath, 'utf8').trim();
      }
    }
...
const nodeEnv = isNodeInstance
  ? {
      ...
      AWS_ENDPOINT_DOMAIN: getValueOrFileContents(
        process.env.AWS_ENDPOINT_DOMAIN
      ),
      ...
      SSE_ENCRYPTION_TYPE: getValueOrFileContents(
        process.env.SSE_ENCRYPTION_TYPE
      ),
