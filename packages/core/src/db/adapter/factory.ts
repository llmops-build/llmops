...
const createAsIsTransaction =
  (adapter: DBAdapter<LLMOpsConfig>) =>
  <R>(fn: (trx: DBTransactionAdapter<LLMOpsConfig>) => Promise<R>) =>
    fn(adapter);

export type AdapterFactory = (options: LLMOpsConfig) => DBAdapter<LLMOpsConfig>;
...
export const createAdapterFactory =
  ...
  (options: LLMOpsConfig): DBAdapter<LLMOpsConfig> => {
      ...
      for (const field in fields) {
        ...
        if (
          ...
          try {
            ...
            child.error('[Adapter Factory] Failed to convert string to date');
          }
        }
      ...
      return where.map((w) => {
        ...
        if (operator === 'in') {
          if (!Array.isArray(value)) {
            throw new LLMOpsError('Value must be an array');
          }
        }
    ...
    const handleFallbackJoin = async <T extends Record<string, any> | null>({
      ...
      try {
        ...
        child.error(`Failed to query fallback join for model ${modelName}:`);
        console.error(error);
        throw error;
      }
      return result;
    ...
    let lazyLoadTransaction: DBAdapter<LLMOpsConfig>['transaction'] | null =
      null;
    const adapter: DBAdapter<LLMOpsConfig> = {
...
function formatTransactionId(transactionId: number) {
  return chalk.magenta(`#${transactionId}`);
}

function formatStep(step: number, total: number) {
  return chalk.bgBlack.yellow(`[${step}/${total}]`);
}
